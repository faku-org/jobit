...
export function JobFit({ job, profile }: JobFitProps) {
  ...
  /* El calce ya está listo al abrir la ficha: el orbe respira, no finge
     que está pensando y después salta de estado. */
  return (
    <Aura className={`rounded-2xl ${HEADLINE_TONE[fit.status]}`} intro={false}>
        ...
        {hasFit(fit) ? (
          <>
            <h3 className="flex items-center gap-1.5 text-sm font-semibold tracking-tight text-ink">
              <AuraSpark className="shrink-0" intro={false} />
              {fitHeadline(fit)}
            </h3>
