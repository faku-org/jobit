  ...
  const submit = (event: React.FormEvent) => {
    ...
    if (mode === "login") {
      return run(async () => {
        ...
        if (result.status === "totp_required") setMode("totp");
    ...
    if (mode === "recover") {
      return run(async () => {
        const recovered = await sendRecover(handle, code);
        onReady(recovered.user);
      });
    }
  };
      ...
      <div className="mt-4 space-y-3">
        {mode === "totp" ? (
          ...
          <>
            ...
            {mode === "recover" ? (
              <Field
                hint="Uno de los que se muestran una sola vez, al crear la cuenta."
