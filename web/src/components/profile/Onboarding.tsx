...
interface OnboardingProps {
  ...
  categories: Facet[];
  departments: Facet[];
  /** False for a browser that already got the welcome and volvió antes de
   * terminar el alta: ya sabe qué es JobIt. */
  showWelcome: boolean;
  onWelcomeSeen: () => void;
...
/**
 * Los pasos que una lectura de CV puede dejar contestados. La lectura se
 * revisa chip por chip antes de aplicarse, así que preguntar de nuevo lo que
 * ya quedó cargado es pedir dos veces lo mismo: esos pasos se saltean y todo
 * sigue editable después desde Perfil y Preferencias.
 */
const ANSWERED_BY_CV: Record<string, (profile: Profile, preferences: Preferences) => boolean> = {
  studies: (profile) => profile.education !== "" || profile.degrees.length > 0,
  skills: (profile) => profile.courses.length > 0 || profile.experienceYears !== null,
  areas: (_profile, preferences) => preferences.categories.length > 0,
  where: (_profile, preferences) => preferences.departments.length > 0,
};

const answeredSteps = (profile: Profile, preferences: Preferences): string[] =>
  Object.entries(ANSWERED_BY_CV)
    .filter(([, answered]) => answered(profile, preferences))
    .map(([id]) => id);
...
export function Onboarding({
  ...
  /** Los pasos que la lectura del CV dejó contestados, sacados del recorrido. */
  ...
  const steps = allSteps.filter((entry) => !skipped.includes(entry.id));
  ...
  /** Read once: what the welcome says does not change, and somebody who ya
   * pasó por acá no está pidiendo que se lo presenten de nuevo. */
  const leaveWelcome = () => {
    onWelcomeSeen();
    setPhase("setup");
  };

  /** Arranca las preguntas sacando del recorrido las que ya están contestadas,
   * así una lectura completa deja solo lo que el CV no puede saber. */
  const startSteps = (answered: string[]) => {
    setSkipped(answered);
    setIndex(0);
    setPhase("steps");
  };
      ...
      <div className="mx-auto flex min-h-svh max-w-lg items-center px-5 py-7 sm:py-10">
        {phase === "welcome" ? (
            ...
            <div className="mt-5 rounded-[22px] bg-panel px-4 py-4 text-onpanel shadow-[var(--shadow-panel)] sm:mt-6">
              <CvImport
                ...
                onApply={(profile, preferences) => {
                  ...
                  startSteps(answeredSteps(profile, preferences));
                }}
                onEmpty={() => {
                  setCvMissed(true);
                  startSteps([]);
          ...
          >
            ...
            {skipped.length > 0 && index === 0 ? (
              <p className="mx-5 mt-2 rounded-lg bg-onpanel-wash px-2.5 py-2 text-[11px] leading-relaxed text-onpanel/70">
                Tu CV ya contestó{" "}
                {skipped.length === 1 ? "una pregunta" : `${skipped.length} preguntas`}, así que las
                salteamos. Lo que cargó queda editable en Perfil y en Preferencias.
              </p>
            ) : null}
