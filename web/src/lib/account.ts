...
export type LoginResult = { status: "ok"; user: Account } | { status: "totp" };
...
export const recover = (
  ...
  code: string,
  password: string,
): Promise<{ status: "ok"; user: Account; recovery_codes: string[] }> =>
  send("/api/auth/recover", { method: "POST", body: JSON.stringify({ handle, code, password }) });

export const logout = (): Promise<{ status: string }> =>
  send("/api/auth/logout", { method: "POST" });

export const fetchMe = (): Promise<{ user: Account; recovery: RecoveryState }> => send("/api/me");
